...
After changing `src/` or lockfile dependencies, commit the rebuilt `dist/` in the same change. CI checks a `src-hash` banner in `dist/index.js` instead of a byte-for-byte ncc diff, because Windows and Linux ncc output is not identical. `dist/` is marked generated so CodeQL scans `src/` rather than the ncc vendor bundle. When bumping the default `php-cs-fixer-version`, update `checksums.txt` with:
